...
export const Utils = {
  ...
  getIp: (req?: import('express').Request): string => {
    ...
    return req.ip || req.socket.remoteAddress || '127.0.0.1:' + config.server.port;
  },
  ...
  /**
   * Pasrse json string or return raw string
   * @param data
   * @returns
   */
  fromJsonOrString(data: string): any {
    try {
  ...
  /**
   * Convert to json string or return raw data if not object
   * @param data
   * @returns
   */
  toJsonOrString(data: any): string {
