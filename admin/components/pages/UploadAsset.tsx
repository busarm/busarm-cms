...
const UploadAsset: React.FC<ActionProps> = (props: ActionProps): JSX.Element => {
  ...
  const onUploadThumbnail = (files: File[], index: Number) => {
    ...
    data[+index].preview = files.length ? files[0] : undefined;
    setUploads(data);
  };
  ...
  const onClick = async () => {
    ...
    // Submit uploads
    if (type) submitMultiple(type, uploads);
  };
  ...
  const submitMultiple = async (type: UploadType, uploads: Upload[]) => {
    if (
      !completed ||
      confirm(
        'Are you sure you want to continue? The upload will be repeated which may lead to duplicates.'
      )
    ) {
      // Refresh status
      if (completed) {
  ...
  const getAssetTypes = async (): Promise<UploadType[]> => {
    const results = await api.resourceAction({
      resourceId: 'AssetType',
      actionName: 'listForUpload',
    });
   ...
   * @return {Promise<UploadType>}
   */
  const getAssetType = async (id: string): Promise<UploadType|null> => {
    const results = await api.recordAction({
      recordId: id,
      resourceId: 'AssetType',
      actionName: 'getForUpload',
    });
    if (results.status == 200) {
          ...
          <Table style={{ minWidth: 1000 }}>
            <TableHead>
              <TableRow>
                ...
                <TableCell>
                  Keywords <div style={{ color: 'gray', fontSize: 11 }}>(comma separated)</div>
                </TableCell>
                  ...
                  <TableRow key={+upload.id}>
                    ...
                    <TableCell style={{ minWidth: 50 }}>
                      <ProgressBar
                        ...
                        onClickReload={() => type && submitSingle(type, upload, index)}
                      />
                    </TableCell>
                    ...
                    <TableCell>
                      <FormGroup>
                        <Input
                          value={String(upload.keywords || '')}
                          style={{ marginLeft: 0 }}
                          onChange={(event: any) => onUploadKeywordsChanged(event, index)}
                          ml="default"
                        />
                      </FormGroup>
                    </TableCell>
