...
/**
 * @param {Object} props
 * @param {String} props.info More info for property
 * @param {String} props.width Image width
 * @returns {JSX.Element}
 */
const ImageViewer = (props: { src: String; width?: Number; height?: Number, onError?: (element: HTMLImageElement)=>any }): JSX.Element => {
    ...
    const onError = (event: React.SyntheticEvent<HTMLImageElement>) => {
        ...
        if(props.onError) {
            props.onError(event.currentTarget);
        }
        setHasFailed(true);
            ...
            {props.src && typeof props.src === 'string' ? (
                <div>
                    <img
                        ...
                        style={{ margin: 'auto', objectFit: 'contain', imageRendering: 'auto', imageResolution: 'from-image' }}
                        alt={props.src}
                        loading={'lazy'}
                        onError={onError}
                    />
                    {isViewerOpen && !hasFailed && (
                        <SimpleImageViewer
                            src={[props.src]}
                            currentIndex={currentImage}
                            disableScroll={false}
                            backgroundStyle={{ zIndex: 1000, background: 'rgba(0,0,0,0.7)', transition: 'all 3s ease' }}
                            onClose={closeImageViewer}
                        />
                    )}
                </div>
            ) : (
                <p style={{ color: 'black', fontSize: 14 }}>No Image</p>
            )}
        </div>
    );
};
