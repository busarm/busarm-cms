...
const ViewJSON: React.FC<BasePropertyProps> = (props: BasePropertyProps): JSX.Element => {
    ...
    const [data, setData]: [Object, Function] = useState(Flat.unflatten(record.params)[property.name]);
    ...
    return (
        <Box style={{ marginTop: 20, marginBottom: 20 }}>
            <Label style={{ display: property.hideLabel ? 'none' : 'block', color: editable ? 'black' : '#898A9A' }}>{property.label}</Label>
            {editable && (
                <Label style={{ color: 'blue', fontSize: 11, display: property.props.info ? 'block' : 'none' }}>{property.props.info}</Label>
            )}
            {editable ? (
                <InputGroup>
                    <ReactJson src={data} onEdit={editable ? onEdit : false} collapsed={false} name={property.name} />
                </InputGroup>
            ) : (
                <ReactJson src={data} onEdit={editable ? onEdit : false} collapsed={false} name={property.name} />
            )}
